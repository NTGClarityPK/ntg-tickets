...
export class AdminService {
  ...
  /**
   * Convert logo base64 data to favicon (32x32 PNG)
   * @param logoBase64 Base64 encoded logo image
   * @returns Base64 encoded favicon image
   */
  private async convertLogoToFavicon(logoBase64: string): Promise<string> {
    try {
      // Decode base64 to buffer
      const logoBuffer = Buffer.from(logoBase64, 'base64');
      
      // Resize to 32x32 PNG (favicon standard size)
      // Using fit: 'contain' to preserve aspect ratio and add padding if needed
      ...
      return faviconBuffer.toString('base64');
    } catch (error) {
      this.logger.error('Error converting logo to favicon:', error);
      throw new Error('Failed to convert logo to favicon. Please ensure the logo is a valid image.');
    }
  }
  ...
  async updateThemeSettings(themeData: {
    ...
    try {
      ...
      // Also auto-generate favicon from logo if logo is being set
      let autoGeneratedFavicon: string | null = null;
      if (themeData.logoData !== undefined) {
        if (themeData.logoData === '') {
          // Empty string means clear the logo data
          mergedData.logoData = null;
          // Also clear favicon when logo is cleared (unless explicitly provided)
          if (themeData.faviconData === undefined) {
            autoGeneratedFavicon = null;
            // Clear favicon data and URL when logo is cleared
            mergedData.faviconData = null;
            mergedData.faviconUrl = null;
          }
        } else {
          mergedData.logoData = themeData.logoData;
          // Auto-generate favicon from logo if faviconData is not explicitly provided
          if (themeData.faviconData === undefined) {
            try {
              autoGeneratedFavicon = await this.convertLogoToFavicon(themeData.logoData);
              this.logger.log('Auto-generated favicon from logo');
            } catch (error) {
              this.logger.warn('Failed to auto-generate favicon from logo, continuing without favicon', error);
              // Continue without favicon if conversion fails
            }
          }
        }
      ...
      // Use auto-generated favicon if available, otherwise use provided or existing
      if (autoGeneratedFavicon !== null) {
        // Use auto-generated favicon from logo
        mergedData.faviconData = autoGeneratedFavicon;
        mergedData.faviconUrl = null;
      } else if (themeData.faviconData !== undefined) {
      ...
      } else if (currentSettings?.faviconUrl !== undefined && themeData.faviconData === undefined && autoGeneratedFavicon === null) {
        // Only preserve faviconUrl if no new faviconData is being provided and no auto-generated favicon
